...
def criar_master_dict(jogadores, jogos, estudios, plataformas, nomes):
    ...
    for jogador in jogadores:
        nickname=nomes.get(jogador)
        
        idJogo, rank = jogadores.get(jogador, (None, None))
        if(idJogo==None):
            continue

        nomeJogo, IdEstudio=jogos.get(idJogo,(None, None))

        if(IdEstudio==None):
            IdEstudio="?"

        nomeEstudio,IdPLataforma=estudios.get(IdEstudio,(None,None))
        if(nomeEstudio==None):
            nomeEstudio="?"

        nomePlataforma,NomeEmpresa=plataformas.get(IdPLataforma,(None,None))

        dic[jogador]={"nickname":nickname,"jogo":nomeJogo,"rank":rank,"estudio":nomeEstudio,"plataforma":nomePlataforma}
    return dic
...
def join_player_jogo_estudio(jogadores, jogos, estudios):
    ...
    for jogador in jogadores:
        ...
        if(idEstudio==None):
            continue
        nomeEstudio,IdPlataforma=estudios.get(idEstudio,(None,None))
        if(jogador == None or nomeJogo == None or nomeEstudio == None):
            continue
        tupla=(jogador,nomeJogo,nomeEstudio)   
        lista.append(tupla)
    return lista
# ------------------------------------------------------------------------------
...
def inverter_jogadores_por_jogo(jogadores):
    ...
    for jogador in jogadores:
        idJogo,rank=jogadores.get(jogador,(None,None))
        if(idJogo==None or rank==None):
            continue

        dic.setdefault(idJogo,[]).append(jogador)
    return dic
...
def inverter_jogadores_por_rank(jogadores):
    ...
    for jogador in jogadores:
        ...
        if(idJogo==None or rank==None):
            continue
        dic.setdefault(rank,[]).append(jogador)
    return dic
...
def calcular_frequencia_estudios(master_dict):
    ...
    """
    dic={}
    for jogador in master_dict:
        print(master_dict.get(jogador,"?").get("estudio","?"))
        estudio=master_dict.get(jogador,"?").get("estudio","?")
        dic[estudio]=dic.setdefault(estudio,0)+1
    return dic
...
def agrupar_por_plataforma(master_dict):
    ...
    dic={}
    for jogador in master_dict:
        plataforma=master_dict.get(jogador).get("plataforma")
        nickname=master_dict.get(jogador).get("nickname")
        dic.setdefault(plataforma,[]).append(nickname)
    return dic
...
# Dicionário: ID da Plataforma s→ (Nome da Plataforma, Empresa)
d_plataformas = {
    'PC': ('Computador', 'Diversos'),
    'STEAM': ('Steam Deck', 'Valve')
}
